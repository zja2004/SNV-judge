# SNV-judge v5：基于基因组基础模型的集成 SNV 致病性预测工具
...
- **Genos 评分**：以人类为中心的基因组基础模型致病性评分 [2]
- **gnomAD AF**：gnomAD v4 人群等位基因频率（ACMG BA1/PM2 信号）
- **Stacking 集成**：XGBoost + LightGBM + 逻辑回归元学习器
- **Isotonic 校准** *(v5 新增)*：比 Platt 缩放更好的概率校准（Brier 0.0685 vs 0.0743）
- **ACMG 5 级徽章** *(v5 新增)*：自动 P/LP/VUS/LB/B 分类，附置信度说明
- **变异历史记录** *(v5 新增)*：会话级查询历史，支持并排 SHAP 对比
- **可靠性图** *(v5 新增)*：模型信息面板中的校准曲线
- **可解释性**：每个变异的 SHAP 特征贡献图
- **🤖 AI 临床报告**：Kimi LLM（Moonshot AI）将所有工具输出综合为结构化 ACMG 风格临床解读报告——现支持**中文 / 英文 / 摘要**三种模板
- **交互式界面**：Streamlit 应用，含致病性仪表盘、评分条形图、SHAP 可视化、历史记录标签页和 AI 报告标签页
- **可复现**：包含完整训练流水线（`train.py`）
...
用户自然语言查询
  ...
  ├── 步骤 3：Genos 智能体    → Stomics Cloud 致病性评分
...
```
SNV-judge/
├── app.py                              # Streamlit 网页应用（v5）
├── kimi_report.py                      # Kimi LLM 临床报告生成（中文/英文/摘要）
├── train.py                            # 完整训练流水线（数据→模型→评估）
├── requirements.txt                    # Python 依赖（含 openai>=1.0）
├── README.md                           # 英文说明文档
├── README_zh.md                        # 中文说明文档（本文件）
├── SNV_judge_opening_defense_v2.pptx   # 开题答辩 PPT
├── xgb_model_v5.pkl                    # 训练好的 Stacking 分类器（v5，8 特征）
...
│   ├── fig_logo_cv.png/svg                 # LOGO-CV 泛化性图（16 基因）← 新增
...
└── skill/
    ├── SKILL.md                            # Skill API 文档
    ├── scripts/
    │   ├── predict.py                      # 离线预测脚本（v5.2，含全部功能）
...
### 2. 设置 API Key（Evo2 + Genos 评分和 AI 报告需要）
...
> Kimi API Key 也可以直接在应用的设置面板中输入，无需重启。
...
# 启用本地 Genos embedding 评分（v5.2 新增，无需 Stomics API Key）
result = predict_variant("17", 7674220, "C", "T", artifacts=artifacts,
...
```python
from skill.scripts.predict import resolve_protein_variant, predict_variant
...
```python
from skill.scripts.predict import load_model_artifacts, predict_vcf
...
# VCF 批量预测 + CSV 输出
python skill/scripts/predict.py --vcf variants.vcf /path/to/SNV-judge --output results.csv

# 蛋白变异名称解析 + ClinVar 查询
python skill/scripts/predict.py --gene TP53 --protein R175H /path/to/SNV-judge --clinvar

# 启用本地 Genos embedding 评分
python skill/scripts/predict.py 17 7674220 C T /path/to/SNV-judge \
...
`data/` 目录中包含训练 v4 所用的所有中间评分：

| 文件 | 内容 | 覆盖率 |
|------|------|--------|
| `scoring_ckpt.pkl` | Evo2-40B LLR + Genos-10B 致病性评分 | 1,677/2,000（83.9%）|
| `vep_scores.pkl` | SIFT · PolyPhen-2 · AlphaMissense · CADD（全部 10,542 个 ClinVar 变异）| 95–100% |
...
#### Genos 致病性评分
Genos 是一个 1.2B–10B 参数的以人类为中心的基因组基础模型，在人类参考基因组和人群变异数据上训练 [2]。通过 `variant_predict` 端点输入 GRCh38 坐标，直接获取致病性概率。
...
- gnomAD GraphQL API 每个变异在应用中增加约 1–2 秒；大型 VCF 建议批量评分
- **MYH7（心肌病）是已知的弱基因**：LOGO-CV AUROC = 0.79，是 16 个测试疾病基因中最低的。三个原因：
  ...
  - **潜在改进**：添加蛋白结构特征（如到肌球蛋白头部 ATP 结合位点的距离、来自 AlphaFold2 结构的结构域间接触能）可能改善 MYH7 区分度。这是 v6 的计划改进方向。
- **未经临床验证，不可用于临床诊断**
